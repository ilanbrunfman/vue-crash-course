...
const jobDropdownItems = ref([
    { title: 'Update', icon: markRaw(IconNotePencil), cta: updateJobModal, rule: true },
...
onMounted( async () => {
    ...
    followers.value = store.users
        ...
        .map(user => ({
            ...user,
            isFollow: true // local follow flag
        })) 
}) 
                        ...
                        <div class="row">
                            <div class="card">
                                <div class="card-header">
                                    <h2 class="sub-title fw-700 text-capitalize">Intro - current user</h2>
                                </div>
                                <div class="card-body">
                                ...
                                <div class="card-body">
                                    <template v-if="user.following.length > 0">
                                        <div v-for="(follower, index) in followers" :key="index" class="card-follower d-flex align-items-center justify-between">
                                            <h3 class="fw-600">{{ follower.firstName }} {{ follower.lastName }}</h3>
                                            <button :class="['btn fw-700 justify-center', follower.isFollow ? 'btn-primary' : 'btn-primary']" @click="toggleFollow(follower)">
                                                {{ follower.isFollow ? 'Follow' : 'Following' }}
                                            </button>
                                        </div>
                                    </template>
                            ...
                            <template v-if="jobs.value.length > 0 ">
                                <div v-for="(job,index) in jobs.value" :key="index" class="card" >
                                    <div class="card-header d-flex justify-between align-items-start">
                                        ...
                                        <button class="btn-horizontal-dots dropbtn" @click="toggleDropdown(job)" :data-dropdown-id="job.id">
                                            <IconHorizontalDots />
                                        </button>
